...
/*   Updated: 2017/11/24 08:43:05 by fpasquer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../incs/client.h"

static int					add_ret(char *ret, unsigned int const curs_nb_wl,
		unsigned int const nb_word_line,
	...
	while (list[i] != NULL)
	{
		...
		while (current_len++ < len_bigger)
			if (ft_strncat(ret, " ", size_ret) != ret)
				return (-1);
		if (add_ret(ret, curs_nb_wl, nb_word_line, COLS % (len_bigger +
				1)) != 0)
			break ;
		curs_nb_wl = curs_nb_wl + 1 >= nb_word_line ? 0 : curs_nb_wl + 1;
	}
