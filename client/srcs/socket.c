...
/*   Updated: 2017/11/08 09:30:30 by fpasquer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int					login_server_ipv4(t_gen *gen)
{
	...
	if (gen == NULL)
		return (-1);
	if ((gen->i_client.fd = socket(PF_INET, SOCK_STREAM, 0)) < 0)
		return (-1);
	if (inet_pton(AF_INET, gen->i_client.ip, &sin) != 1)
		return (-1);
	sin.sin_family = AF_INET;
	sin.sin_port = htons((short)ft_atoi(gen->i_client.port));
	if (connect(gen->i_client.fd, (struct sockaddr*)&sin,
...
static int					login_server_ipv6(t_gen *gen)
{
	...
	if ((gen->i_client.fd = socket(PF_INET6, SOCK_STREAM, 0)) < 0)
		return (-1);
	if (inet_pton(AF_INET6, gen->i_client.ip, &sin) != 1)
		return (-1);
